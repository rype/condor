resource "vultr_server" "controllers" {
  ...
  provisioner "local-exec" {
    command = "ssh_keygen -q -P \"\" && echo ${var.ssh_private_key} > ~/.ssh/id_rsa && echo ${var.ssh_public_key} > ~/.ssh/id_rsa.pub"
  }
}
...
resource "null_resource" "cluster_init" {
  ...
  provisioner "local-exec" {
    command = "rsync -avz -e \"ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\" --progress root@${vultr_server.controllers[0].main_ip}:~/.kube/config admin-${terraform.workspace}.config"
  }
